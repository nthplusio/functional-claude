...
- **Before messaging a teammate about a task, verify it is unblocked. Call `TaskUpdate` to clear `blockedBy` first — never send a task assignment message while the task still shows as blocked. Mixed signals (assigned + blocked) cause teammates to stall or start prematurely.**
