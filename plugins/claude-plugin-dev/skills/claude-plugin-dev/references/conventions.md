...
### Cross-Platform Shebang (Critical for .js hooks)

**Never use `#!/usr/bin/env node`** — fails on macOS with version managers (nvm, fnm, volta). Use the polyglot preamble:
...
For simple validation/logging hooks, prefer pure bash with `jq` to avoid this entirely.
